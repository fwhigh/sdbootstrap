...
\tikzstyle{storage} = [cylinder, minimum width=3cm, minimum height=1cm, text centered, shape border rotate=90, aspect=0.2, draw=black]
...
\node (in1) [io] {Streaming data queue $\{(X_1,w^{\prime}_1), (X_2,w^{\prime}_2), ...\}$};
\node (pro1) [process, below of=in1] {Inner bootstrap};
\node (sto1) [storage, below of=pro1] {$r$ bootstraps};
\draw [arrow] (in1) -- (pro1);
\draw [arrow] (pro1) -- (sto1);
\end{tikzpicture}
